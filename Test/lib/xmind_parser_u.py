...
def parse_xmind_content():
    ...
    root_suite.sub_suites.sub_suites = []
    suite_nodes = children_topics_of(xml_root_suite)

    sub_suite_node = children_topics_of(suite_nodes)

    if suite_nodes is None:
        raise ValueError("Cannot find any test suite in xmind!")

    for sub_node in sub_suite_node:
        sub_suite = parse_suite(sub_node)
        root_suite.sub_suites.sub_suites.append(sub_suite)
