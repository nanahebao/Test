...
print(project)
with o.execute_sql("""select Acvalue, 
Ccvalue,
Dcvalue,
count(1) num
from 
...
from
    (select carno from (
        select carno from (
            select carno from (select *,row_number() over(partition by carno order by createtime desc ) rn from  ofo_t_order_state where dt<=20171220
            ) a where rn=1 and dt<20171218
    union all
    ...
    left outer join
    (select carno from  ofo_t_order_hg where dt=20171221
...
from
    ...
    right outer join
    (select carno from (select *,row_number() over(partition by carno order by createtime desc ) rn from  ofo_t_order_hg where dt<=20171221
            ) a where rn=1 and createtime < '2017-12-19 00:00:00' ) t_today_silent on t_last_day_silent.carno = t_today_silent.carno
where t_last_day_silent.carno is null
    ) AA
union all
  select tt.carno, 'silent_tag' as cname,'continue_acitve' as cvalue from (
select carno from ofo_t_order_hg where dt>='20171218' group by carno
) tt left outer join
(select carno,'silent_tag' as cname, tag as cvalue from (
select t_last_day_silent.carno,
    case when  t_today_order.carno is not null and t_engine_jd.bicycle_no is null then 'zr_active'
...
left outer join (
select carno from  ofo_t_station_record where dt<='20171221' and dt>='20171214' and record_type=1
group by carno
) SS on A.carno = SS.carno
where SS.carno is  null
) AAA where Bcvalue = 1 group by Acvalue , Dcvalue, Ccvalue""").open_reader() as reader:
