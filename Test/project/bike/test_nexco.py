...
(select A.carno,
    ...
    'rail_type' As Fcname, case when F.type=3 then '外围' else '核心' end as Fcvalue
 from
 (select carno, 'city' as cname, city as cvalue, area from  ofo_t_bike_location where dt=20171222 ) A
left outer join
(select carno, 'order_num' as cname, 1 as cvalue from  ofo_t_bike_location where dt=20171222
...
left outer join
(select carno, 'lock_type' as cname, lock_type as cvalue from  ofo_t_bike_location where dt=20171222
...
from
    ...
    left outer join
    ( select bicycle_no from ofo_t_bicycle_mark_records where dt=20171222 and tags like '%silence%' and status=5 group by bicycle_no) t_engine_jd
...
from
    ...
    union all
    select bicycle_no as carno from ofo_t_bicycle_mark_records where dt=20171222 and tags like '%silence%' and status=5 group by bicycle_no ) tmp group by carno ) t_last_day_silent
    right outer join
